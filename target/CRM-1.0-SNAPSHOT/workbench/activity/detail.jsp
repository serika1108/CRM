			...
			$.ajax({
				...
				success: function (data) {
					...
					if(data.success){
						...
						//添加成功，在文本域上面新增一个div
						let html="";
						//javascript:void(0);禁用超链接，动态生成的按钮使用触发事件的方式
		...
		$("#updateRemarkBtn").click(function () {
			...
			$.ajax({
				...
				success: function (data) {
					/*
					data
						{"success":true/false，"ar":备注的对象}
					 */
					if(data.success){
						//修改备注成功，更新div中的相应信息：noteContent，editTime，editBy
		...
		$.ajax({
			...
			success: function (data) {
				...
				$.each(data, function (i, n) {
					...
					/*
					javascript:void(0) 禁用超链接，只能以触发事件的形式操作
					a标签能直接触发事件，用onclick属性指定事件
					动态触发的事件传递的参数要用套用在字符串中，即参数左右要有引号的引用，而不能直接进拼接
					静态标签触发的事件可以拼接，不用引号的引用
					*/
	...
	function deleteRemark(id){
		...
		$.ajax({
			...
			success: function (data) {
				...
				if(data.success){
					/*
					删除后应该得到新的备注信息列表，注意这里不能直接调用函数showRemarkList()
					因为showRemarkList的原理是在标签前追加数据，而不是清空备注列表重新显示
					因此直接调用会导致越删越多，因为之前的备注信息被保留了下来
					因此需要将要删除的div加一个动态的id，id会当做参数传进来，根据id寻找并删除
					 */
